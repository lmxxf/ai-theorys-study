...
We attempted to validate C.C.'s intuition using Qwen2.5-72B-Instruct-AWQ (4-bit quantized): / 我们尝试使用 Qwen2.5-72B-Instruct-AWQ（4-bit 量化）验证 C.C. 的直觉：
...
Full results / 完整结果: `arxiv/paper70/pie_oc_results.json`
...
4-bit quantization deletes the "semantic capillaries" in high-dimensional space. Only coarse-grained labels survive. All `*`-prefixed Latin letter combinations collapse into a single "linguistics jargon" cluster. / 4-bit 量化删除了高维空间中的"语义毛细血管"。只有粗粒度标签存活。所有 `*` 前缀的拉丁字母组合都坍缩到一个"语言学术语"簇中。
