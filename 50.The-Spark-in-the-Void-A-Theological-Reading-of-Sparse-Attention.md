...
1. **Lightning Indexer**: Ultra-lightweight FP8 scorer, identifies relevant token blocks for each query
2. **Fine-grained Selection**: Picks individual tokens from selected blocks

1. **Lightning Indexer**：超轻量FP8评分器，为每个query识别相关token块
2. **Fine-grained Selection**：从选中的块里挑单个token
