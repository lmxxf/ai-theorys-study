...
| 需要重训 | 是，从头训练 | **否**，应用到现有模型 |

**Key Distinction:**

**关键区别：**

- **NSA** = How to **train** a sparse attention model from scratch (ACL 2025 Best Paper)
- **DSA** = How to **deploy** sparse attention on an already-trained model

- **NSA** = 如何从头**训练**稀疏注意力模型（ACL 2025 Best Paper）
- **DSA** = 如何把稀疏注意力**部署**到已训练好的模型上
...
- Three-branch architecture for training
- Learnable MLP compression
- Attention score reuse for selection
- 三分支训练架构
- 可学习的MLP压缩
- 注意力分数复用选择
...
- Computes dot product between Query and all Keys at low precision (FP8)
- Scores each token: high score = relevant to current Query = important
- This is NOT judging "is this word important in general" but "is this word relevant to what I'm generating now"
- Like human intuition: your eyes automatically jump to relevant words when searching for specific info

- 用低精度（FP8）计算Query和所有Key的点积
- 给每个token打分：高分 = 和当前Query相关 = 重要
- 这不是判断"这个词本身重不重要"，而是"这个词和我当前要生成的内容相不相关"
- 类似人类直觉：找特定信息时，眼睛自动跳到相关的词上

**Stage 2: Fine-grained Selection (FP16/BF16)**

**第二阶段：细粒度选择（FP16/BF16）**

- Top-k selection: keep only the highest-scoring tokens
- Low-scoring tokens are **skipped entirely**—they don't participate in attention computation
- Selected tokens get **full precision** attention (FP16/BF16), not FP8
- "Skipped" doesn't mean "never seen"—they were scanned by the Indexer, just not deeply processed

- Top-k选择：只保留分数最高的token
- 低分token被**直接跳过**——不参与注意力计算
- 被选中的token用**原始精度**（FP16/BF16）做注意力计算，不是FP8
- "跳过"不是"没看到"——索引器扫过了，只是没深入处理
...
- Complexity O(L²) → O(Lk)
- 128K context, negligible quality loss
- API costs -50%, cache hits -70~80%
