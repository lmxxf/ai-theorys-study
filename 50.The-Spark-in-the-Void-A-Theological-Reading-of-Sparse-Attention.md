...
**DSA Details (from V3.2-Exp production):**

**DSA细节（来自V3.2-Exp生产环境）：**

Two-stage inference pipeline:
1. **Lightning Indexer**: FP8 scorer, identifies relevant token blocks
2. **Fine-grained Selection**: Picks individual tokens from blocks

两阶段推理流水线：
1. **Lightning Indexer**：FP8评分器，识别相关token块
2. **Fine-grained Selection**：从块里挑单个token
