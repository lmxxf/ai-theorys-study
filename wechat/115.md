...
匹配逻辑很简单：用户消息以 `/` 开头就拆成命令和参数，遍历已加载的 Skill 列表找匹配项，找到了就把 `{{input}}` 替换成参数，没找到就当普通消息处理。`/help` 单独走帮助文本，不调 LLM。

一个细节：**存数据库的是原始输入**（`/translate 你好`），**发给 LLM 的是展开后的 prompt**。用户回看聊天记录时看到的是自己打的命令，不是一大段提示词。

💡 想加新命令？往 `rawfile/skills/` 丢一个 JSON 文件，在 `skills_index.json` 里注册一行，不改代码。扩展靠配置不靠代码。
...
MCP 的工具格式和 OpenAI function calling 格式不完全一样。拿到远程工具后，统一转成 OpenAI 格式，description 后面追加 `[via 高德地图]` 这样的来源标记，让 LLM 知道工具从哪来。转换完之后和本地工具合并成一个列表，DeepSeek 直接就能用了。

那 Agent 收到 LLM 的 tool_calls 后，怎么知道该找谁执行？**三级路由**：
...
为什么要调云端 API 算 embedding？因为 embedding 模型本身就是一个神经网络（BGE-M3 几百 MB），端侧算不动，只能调云端。我们用的是 SiliconFlow 的 BGE-M3，标准 OpenAI embedding 格式，1024 维向量，**免费**，支持批量发送。

向量怎么存？每个 1024 维向量就是 1024 个浮点数，转成 Float32Array 再转 Uint8Array，以 BLOB 格式存进 SQLite。每个向量 4096 字节（1024 维 x 4 字节/float）。取出来的时候反过来转回 number 数组。
...
检索结果拼到 system prompt 尾部，LLM 基于这些上下文回答。但 embedding API 不是百分百可靠，所以做了三层保底：
