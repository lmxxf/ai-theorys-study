吃完晚饭，突然发现昨天做的 OHClaw 还明显缺功能，于是拉上 Claude 一起继续干活儿。

缺什么？三个东西：

1. **Skill 斜杠命令** — 用户输入 `/translate 你好` 直接翻译，不用说"请帮我翻译"
2. **MCP Client** — 接入外部工具服务，比如高德地图，Agent 自动调用路线规划、天气查询
3. **RAG 向量检索** — 端侧知识库，Agent 回答问题时自动检索相关内容
