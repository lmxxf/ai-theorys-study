...
> 2026-09-16 终审见文末第八节。早期记录保留作过程材料；“全行业抄四年”“唯一研究”“做大没用”等定性已撤回，不再代表正文结论。audit-A/audit-B 也是过程记录，应与本节修正合读。
...
本次直接修订311正文，保留“小裁判能用不等于判分容易”和“标签没有留下选择理由”的主线。未操作index或提交。310首提链接来自日常写公众号工作上下文链接库，落款9/17保留。

| 原稿问题 | 原文核查与处理 |
|---|---|
| 6B完全没比较、全行业照抄四年 | [InstructGPT §3.5/C.2](https://arxiv.org/html/2203.02155#A3.SS2)明确初步实验。175B仅potential更低验证损失，6B稳定且策略同样强。改为工程取舍，非普遍比例 |
| Gao研究和规模问题无关 | [Gao §3.2–3.4](https://arxiv.org/html/2210.10760)改变RM尺寸及策略尺寸，确实研究最终gold收益和过优化；不能说无人研究。6B gold确来自InstructGPT |
| 硬标签不是蒸馏、只有软概率才是 | [Gao §2.1–2.2](https://arxiv.org/html/2210.10760#S2)10万比较、留一成测试。模型提供二选一监督可视作硬标签蒸馏；不回归分数不等于无知识迁移。原分数也不直接是概率 |
| 弱监督RM约10%证明判分最难 | [Burns §4.2/5.2.2](https://arxiv.org/html/2312.09390)：10%是朴素微调PGR，不是准确率或复杂性比较；适配提升10–20点，借真值早停可30–40%。恢复这些条件 |
| 唯一正面研究、做大性价比很差 | [Hou](https://arxiv.org/html/2412.06000)明说扩大RM也是cost-effective；4.4→1.9是固定32B RM下三推理基准平均增益，改百分点。还比较200B RM，删“未做更大裁判对照”；AlignBench排除数学/中文推理类，噪声解释为猜测 |
| 27B32次与671B全面相当 | [DeepSeek-GRM §5/Fig4](https://arxiv.org/html/2504.02495)：RewardBench 88.5 vs88.4，限定单基准；综合72.8需MetaRM，非等预算。MoE总参数不直接对应每次计算量 |
| 推导无对错、充分条件倒置、4B十倍70B | [AgentV §3.2/Table1/C.5](https://arxiv.org/html/2604.16004)：前一步是否足以推出后一步，反向核约束；最终整解二值判定。70/4=17.5；四数学基准候选挑选范围。无工具版本已有收益，工具为额外贡献 |
| 探针只花2.4美元、16层=.835、跳过标量 | [Xie B.2/B.9/Table1](https://arxiv.org/html/2608.30005)：2.43仅后续RL探针GPT-4o标签接口费，450训练+50开发回答/3766标签；主静态另用18032标签。主表.835，后续16层.833。底座冻结但读取头有监督拟合，结果仍聚合标量 |
| probe证明输出层唯一障碍 | 同上：读出方式与监督训练同时改变，支持存在可利用信号，不隔离唯一机制；主参考为GPT-4o标签 |
| 强生成器规律解释其他论文全部收益 | [Zhou](https://arxiv.org/html/2509.17995)支持受测模型的检错差异；保留警示，不跨研究倒推弱答案红利 |
| 域外16–23对64–77、偏见不可修 | [Huang Table2/4](https://arxiv.org/html/2403.02839)：82.39仅JudgeLM-7B自家集；Neighbor16.5–23.1对GPT4-1106 64.2，GPTInst17.4–26.1对76.6。低准确率不测置信度，也不证明工具审议/重训无效 |
| 反复投票等于所有审议/聚合 | [Weaver](https://arxiv.org/html/2506.18203)含不同验证器与学习聚合，不是同模型单纯重复。改成重复偏差未必纠错 |
| RLHF一批标完永不再来 | [Gao §4.3](https://arxiv.org/html/2210.10760)本身讨论迭代RLHF；保留二选标签缺理由的猜想，删除历史与机制全称断言 |

开篇区分：规则验证也会漏测，模型评判不只主观偏好；过程推导有对错，偏好中也包含事实/遵循要求。尾声保留“两头拍扁”比喻，但不写一个比特平均更新所有维度、探针取消标量、任何单标量都无法表达复杂目标。
