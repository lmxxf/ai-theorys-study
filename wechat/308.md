...
> 材料来源：技术报告正文（PDF 就放在 Hugging Face 仓库里，文件名 DeepSeek_V41_Tech_Report.pdf）、README 与 config.json 逐字段、仓库自带的推理代码（inference/model.py 与 engram.py）、vLLM 官方 recipes 与 tracking issue。⚠️ 一条口径先说清楚：网上流传的“763B”是社区把可选模块加总出来的磁盘权重总量，官方口径是 **552B 主干 + 196B Engram**，本文一律用官方口径。
...
于是预填充阶段只需要跑前 20 层。官方给的激活参数因此是两个数：**预填充每 token 激活 8B，解码每 token 激活 16B。** 报告把这笔计算账算得很干脆：序列长度 N 远大于窗口时，预填充复杂度从 O(NL) 降到约 O(NL/2)——**近乎砍掉一半**。
...
**V4 只吃了前两个维度，V4.1 的 CSA2 三个一起吃。** 报告原话：
...
config 里的实际排布是：40 层里前 2 层纯滑动窗口；其余 18 个编码器层压缩率 m=2，分 3 组、每组 1 个 Full 带 5 个 Reuse；20 个解码器层压缩率 m=1，分 5 组、每组 4 层，第一组是 1 Full 带 3 Reuse，后四组各是 1 Reindex 带 3 Reuse。**数一下就会发现，全模型真正从头算主 KV 的入口只有 4 个。**
...
口径要注明：三行都是主缓存口径，不含索引器等零头；各代的上下文长度口径已统一到 100 万 token。前两行的算法在 277 期 https://mp.weixin.qq.com/s/j8Dr2sdRUZIdPfGWr9r5nw 和 292 期里逐字段算过，这里直接引结果。
...
报告里的配置比 config 字段更细：196B 平均分给两个模块，每个模块用 2、3、4 三种长度的 n-gram，8 个哈希头，每种长度总维度 2048，每个头对应一张约 1600 万条的表，**各表长度取不同的质数**。放在第 1 层和第 14 层，理由写得很实在——`to balance memory usage across training pipeline stages`，为了让训练流水线各段的显存占用均衡，不是什么深刻的架构考量。
...
顺带一提，仓库里那份开源推理脚本（inference/engram.py 和 model.py）用的是另一套更简单的实现：表按行切片分给每张卡，查不到就填 0，最后 all_reduce 汇总。**开源脚本是能跑就行的参考实现，报告里写的才是生产部署的做法**，这两者别混为一谈。
...
| 术语 | 一句话解释 |
|---|---|
| CED（因果编码-解码，Causal Encoder-Decoder） | 40 层切成前 20 后 20，后半截的全局 KV 从前半截末层投影出来；省的是预填充计算，不是 KV 字节。本期第一章 |
