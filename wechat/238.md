【在 50 系显卡上实现 DeepSeek V4 算子】算力的主力——fp8_gemm 和 fp4_gemm 怎么把 dequant 融进矩阵乘
...
上一期讲了 `act_quant` 和 `fp4_act_quant` 把激活量化成了 fp8/fp4。今天我们继续走 169 期那条 token 路径——既然激活已经量化好了，下一步就是真正的算力主力：矩阵乘法。

V4 推理里 99% 的算力都耗在矩阵乘上。MLA 的 Q 投影、KV 投影、O 投影，MoE 每个专家的 SwiGLU 上下投影，每一发都是一次矩阵乘。V4 一共 61 层、每层 384 个专家（V4 已升级，按 DeepGEMM 默认配置），每个 token 走一遍 attention + MoE，光是一次 forward 就要点几万次 GEMM。
...
- `fp8_gemm`（第 190–333 行）：fp8 激活 × fp8 权重 → bf16
- `fp4_gemm`（第 343–429 行）：fp8 激活 × fp4 权重 → bf16，用了新 API `tl.dot_scaled`
- `_dequant_fp4`（第 334–342 行）：CPU 兜底用的查表 dequant

为了避免误解，先把上下文交代清楚——本系列的"V4"指的是我们在 DGX Spark 上跑的 V4 Flash 280B 实验平台，仓库在 `~/work/deepseek-v4-experimental-platform-on-dgx-spark/`。sm_121 是 DGX Spark 的 GPU 架构编号（消费级 Blackwell 家族）。
...
加粗那一行就是大头：**为了"还原"成 bf16，专门实例化了一个和 A 等大的中间张量，往 HBM 写一遍再读一遍**。fp8 省下来的访存优势，全在这一步还回去了。

V4 的 MoE 每层激活 384 个专家中的 6 个，每个 token 每层要跑 6+ 次专家 GEMM。**假设激活实例化中间张量，每次都要往 HBM 写一份 bf16 中间结果**，整层下来访存压力翻倍，sm_121 这种消费级 Blackwell 本来访存带宽就被砍过，更扛不住。
...
182 期那篇勘误已经说清楚了：**sm_121 上 FP8 mma 硬件指令是完整存在的**——`mma.sync.aligned.m16n8k16.f32.e4m3.e4m3.f32` 这条指令可用。我们之前以为"FP8 也残了"，是从"FP4 不行"顺手脑补的。
...
整个 fp8_gemm 的 inner loop，精华就这 5 行（`_fp8_gemm_kernel` 第 160–181 行）：
...
for k0 in range(0, K, BLOCK_K):       # 沿 K 维度滑动
    ...
    b_s = tl.load(b_scale_ptr + ...)   # 这一块 K 对应的 scale（每列一个标量）
...
![两条路线：先 dequant 整张矩阵 vs 边读边乘 scale](images/238/fused_dequant.svg)

![每个 program 算 C 的一个 tile：沿 K 流水累加](images/238/block_tile.svg)
...
`_dequant_fp4`（第 334–342 行）是给 CPU/没有 Triton 的环境兜底用的——把 fp4 字节按高 4 位/低 4 位分开，查 FP4_TABLE 还原成 fp32：
...
Attention 这一侧的权重精度高（Q/KV/O），走 fp8。**MoE 专家权重数量大、走 fp4**——V4 一层 384 个专家，每个专家两个 SwiGLU 矩阵，参数量在整个模型里占绝对大头。fp4 比 fp8 又省一半访存和存储，再叠加 dot_scaled 的硬件级 dequant，MoE 推理才能在 DGX Spark 这种消费级硬件上跑起来。

158 期讲过：DeepSeek 自己的 DeepGEMM 里 MoE 路径就是用 fp8 activation × fp4 weight。我们在 sm_121 上抄的是同一套接口设计——`fp8_fp4_gemm`——只是底层走了 Triton 替代 DeepGEMM 的 SM100 专用代码。
...
1. **量化 GEMM 的关键不是算得快，是读得少**。fp8/fp4 的核心收益在访存——HBM 带宽是 sm_121 的瓶颈，每个元素少 1 byte 就少 1 byte。这条收益一旦中间被实例化成 bf16 张量就全没了，所以才需要 fused dequant。
...
- 237 期：act_quant 和 fp4_act_quant（量化的入口）
- 169 期：一个 token 的旅程——V4 全流程总复习
- 182 期：sm_121 硬件支持状况勘误（FP8/FP4 mma 实际完整存在）
- 158 期：Mega MoE 和 FP4 Indexer——V4 发布前的两记重拳
