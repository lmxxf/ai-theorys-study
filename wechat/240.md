...
为了避免误解，先把上下文交代清楚——本系列的"V4"指的是我们在 DGX Spark 上跑的 V4 Flash 280B 实验平台，仓库地址 https://github.com/lmxxf/deepseek-v4-experimental-platform-on-dgx-spark 。sm_121 是 DGX Spark 的 GPU 架构编号（消费级 Blackwell 家族）。**`fp8_gemm`、`fp4_gemm` 这些是我们自己为了在这块消费级卡上跑通 V4 而写的实验性替换算子，不是 DeepSeek 官方产品，也不是 NVIDIA 官方方案**——官方原版用的是 TileLang 写的 kernel，跑在数据中心卡上；我们这套是自己用 Triton + PyTorch 另起炉灶的替代实现，只为了在 DGX Spark 上把研究做起来。
...
A_bf16 = (A_fp8.float() * a_scale.unsqueeze(-1)).to(torch.bfloat16)
B_bf16 = (B_fp8.float() * b_scale.unsqueeze(-1)).to(torch.bfloat16)
...
先说个前提：182 期《重新理解 SM120+ 的硬件支持状况——我们之前搞错了》（ https://mp.weixin.qq.com/s/dMamjfJwB2A5cCq6Xw4qCQ ）勘误已经查清楚了——**sm_121 上 FP8 mma 硬件指令是完整存在的**，PTX 层面 `mma.sync.aligned.m16n8k32.f32.e4m3.e4m3.f32` 这条指令可用，从 sm_89（RTX 4090 那一代）就有。当时判断过"sm_120+ 消费级卡的 FP8/FP4 硬件残废"，后来验证是判断错了——硬件指令一直都在，缺的是把它用起来的软件封装。
...
整个 fp8_gemm 的 inner loop，精华就这 5 行（`_fp8_gemm_kernel`）。先说三个下标变量：`offs_m` 是这个 program 负责的那 BLOCK_M 行 token 编号，`offs_n` 是负责的那 BLOCK_N 列输出通道编号，`k` 是这一步在 K 维上滑到的那一段位置——三个都是提前算好的下标数组，不是循环变量。

代码里的 `expand_dims(axis)` 就是"在指定位置插入一个长度为 1 的新维度"——PyTorch 里这个操作叫 `unsqueeze`，Triton 的张量类型用的是 `expand_dims` 这个名字。**注意，这不是"同一个函数换了个马甲"——PyTorch 的 tensor 和 Triton 的 tensor 是两套完全不同的类型系统，互相不认识，写错了对方也不会提醒你。** 只是恰好两边都需要解决同一个问题（把一维数组立起来或摆平，好让两个方向不同的数组能广播出一整块二维网格），所以各自发明了一个长得不一样的解法。下次见到不认识的方法名，先确认自己在哪个库的地盘里，不要凭直觉套用另一个库学过的写法：

```python
for k0 in range(0, K, BLOCK_K):       # 沿 K 维度滑动，一次走 128
    k = k0 + offs_k                    # 这一步覆盖的 128 个 K 位置

    a   = tl.load(a_ptr + offs_m.expand_dims(1) * a_stride_m + k.expand_dims(0) * a_stride_k)
    # 按"第几行 token、第几个 K 位置"这两个下标，取出 BLOCK_M × BLOCK_K 的 fp8 片

    b   = tl.load(b_ptr + offs_n.expand_dims(1) * b_stride_n + k.expand_dims(0) * b_stride_k)
    # 同样按"第几列输出通道、第几个 K 位置"，取出 BLOCK_N × BLOCK_K 的 fp8 片

    a_s = tl.load(a_scale_ptr + offs_m * as_stride_m + (k0 // BLOCK_K) * as_stride_kb)
    # 这一块 K 对应的 scale，每行（每个 token）一个标量

    b_s = tl.load(b_scale_ptr + (offs_n // 128) * bs_stride_nb + (k0 // BLOCK_K) * bs_stride_kb)
    # 这一块 K 对应的 scale，每 128 列输出通道共用一个标量

    acc += tl.dot(a, tl.trans(b), out_dtype=tl.float32) * a_s.expand_dims(1) * b_s.expand_dims(0)
```

**提醒一句：上面为了讲清楚特意把 `expand_dims`（以及第一节里的 `unsqueeze`）写全了，但实际源码里几乎没人这么写，通常会写成简写形式 `offs_m[:, None]`、`a_s[:, None]`（Triton）、`a_scale[..., None]`（PyTorch）——中括号里塞一个 `None`，效果和 `expand_dims`/`unsqueeze` 一模一样，只是更短。以后不管在 PyTorch 还是 Triton 代码里见到方括号里夹着一个 `None`，认出它就是这里讲的这件事就行——但记住它们仍然是两套不同的东西，只是写法上都借了这个简写。**
