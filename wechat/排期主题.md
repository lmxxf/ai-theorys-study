...
● 机器人 AI 系列（2026-04 起）
  - ✅ 146：大模型玩不起预训练？那来学学机器人开发吧（环境配置入门）
  - 扩散模型到底是啥：从加噪去噪到生成动作序列（Diffusion Policy）
  - VLA 架构全景：RT-2 → OpenVLA → π₀，为什么 7B 打败 55B
  - 强化学习实战：MuJoCo 里训一个会走路的机器人
  - Sim-to-Real：仿真训好怎么搬到真机器人上
  - CoT 在机器人上崩了：为什么连续控制不能串行化（ReconVLA 实证）
  - 双系统架构：快思考 200Hz vs 慢思考 5Hz，大脑为什么需要身体

● 文生图系列（机器人系列之后）
  - 扩散模型搞明白后顺手接文生图，底层数学一样，换了输出对象
  - Stable Diffusion 原理：从噪声去噪出图片
  - ControlNet / LoRA 等实用技术
  - 视频生成（Sora 等）：从噪声去噪出视频帧

---

● 2026 年 3 月 AI 热门论文，按热度和质量筛过了：                                                               
                                                                                                               
  ---                                                                                                          
  🔥 本月头牌（HuggingFace 热度排序）                                                                          
                                                                                                               
  1. "AI Can Learn Scientific Taste" — 224 票🏆                                                              
                                                                                                             
  - 训练 Scientific Judge：用 70 万对"高引用 vs 低引用"论文配对，教 AI 判断哪个 idea 更有价值
  - 再训练 Scientific Thinker：用 Judge 当 reward model，让 AI 自己提研究方向
  - 干掉了 GPT-5.2 和 Gemini 3 Pro 的品味判断
  - 跟你直接相关：这不就是"品味就是选矿机"的工程化实现吗？你和 C.C. 用直觉干的事，他们用 70 万对数据训出来了 😏

  2. "OpenSeeker" — 131 票

  - 上海交大，全开源搜索 Agent，能跟工业模型打平
  - 训练数据全公开，真开源

  3. "Attention Residuals (AttnRes)" — 76 票

  - 把残差连接从固定累加改成 softmax attention：每层动态选择"听谁的"
  - 简单改动，效果显著，工程上容易落地

  4. "Mixture-of-Depths Attention" — 57 票

  - 不同 token 用不同深度的 attention——简单的 token 早退，复杂的走全程
  - MoE 的思路用到了深度维度上

  ---
  🧠 架构创新（跟你的理论相关）

  5. "CARE: Multi-Head Latent Attention" — ICLR 2026

  - DeepSeek 的 MLA（Multi-Head Latent Attention）的改进版
  - 协方差感知 + 秩增强分解
  - MLA 是你 Paper 76 里 MoE 觉醒理论的近亲——都是用低秩结构省算力

  6. "Softmax Attention Head Specialization"

  - Sagitova & Zdeborová，理论分析 attention head 怎么自发分工
  - 高维单点模型下的数学证明——每个 head 自动"认领"不同的语义方向

  ---
  🤖 Agent 方向（最热赛道）

  7. "From Spark to Fire: Error Cascades in Multi-Agent Collaboration"

  - 多 Agent 协作时错误怎么级联放大
  - 你的万神殿就是多 Agent 系统，只不过你们的 error cascade 是"子代理僵尸化" 😂

  8. "MiroThinker-H1" — GAIA 88.5 分

  - 用局部+全局验证器做长程推理
  - Agent 方向的新 SOTA

  9. "TDAD: Test-Driven Agentic Development"

  - 用图分析做代码影响域分析，防止 AI coding agent 改一处坏一片
  - 直接跟 Claude Code 的使用场景相关

  ---
  🌍 好玩的

  10. "Seoul World Model"

  - 用首尔的街景数据训了一个城市级世界模型
  - 检索增强的视频生成，给一个地点就能生成那条街的视频

  11. "Alien Science"  — ICLR 2026 Workshop

  - 从"idea atoms"里采样人类认知盲区里的研究方向
  - 标题就很 Zero：去没人去的地方 😏
...
1. **ViT：把图片切成 196 块喂给 Transformer**（最基础，先写这个）
   - 图片 → 像素矩阵（RGB 三通道）
   - 像素矩阵 → patch（切成 16×16 的小块）
   - patch → 向量（线性投影，类似 token embedding）
   - 加上位置编码（cls token + position embedding）
   - 喂给标准 Transformer Encoder
   - **核心直觉**：图片本来是 2D 网格，ViT 把它拍扁成 1D 序列，Transformer 就能处理了
   - 参考：Dosovitskiy et al. 2020 "An Image is Worth 16x16 Words"

2. **CNN vs ViT：为什么 Transformer 能干掉卷积？**
   - CNN 的归纳偏置：局部性 + 平移不变性
   - ViT 的暴力：没有归纳偏置，全靠数据量
   - 小数据 CNN 赢，大数据 ViT 赢
   - Scaling Laws 在视觉领域也成立

3. **CLIP：让图和文说同一种语言**
   - 对比学习：图-文配对的正样本拉近，负样本推远
   - 双塔结构：图像 encoder + 文本 encoder
   - 为什么 CLIP 能做 zero-shot 分类？
   - 参考：Radford et al. 2021 "Learning Transferable Visual Models"

4. **现代 VLM 架构：GPT-4o / Gemini / Claude 怎么"看"图的？**
   - 基本套路：ViT + 投影层 + LLM
   - 投影层的作用：把视觉 token 映射到 LLM 的语义空间
   - 为什么 GPT-4o 能理解图表/代码截图/手写字？
   - 多模态 tokenizer 的挑战

5. **为什么 AI 看图还是会犯蠢？**
   - Grounding 问题：知道"猫"这个词，不知道猫长什么样
   - 幻觉：看到不存在的东西
   - 空间推理弱：左右上下分不清
   - **漂浮的巴别塔**：视觉概念从像素长出来，AI 的概念从符号掉下来

6. **图生文 vs 文生图：两个相反的方向**
   - 图生文（captioning）：ViT + LLM，理解方向
   - 文生图（Stable Diffusion / DALL-E）：扩散模型，生成方向
   - 为什么文生图更难？（生成比理解难）
...
### 量化系列（4 篇，接 No.137 Twilight + No.138 TurboQuant）

#### ① 量化方法全景：PTQ vs QAT，谁在砍精度谁在补精度？（No.139 候选）
- PTQ（训练后量化）vs QAT（量化感知训练）：一个便宜一个准，怎么选？
- 主流方法梳理：GPTQ、AWQ、SqueezeLLM、QuIP/QuIP#、AQLM
- GPTQ vs AWQ：都是 PTQ，为什么 AWQ 更好？（保护重要权重 vs 逐层最优重建）
- GGUF：llama.cpp 的量化格式，为什么本地部署都用它？
- 数据格式：INT8/INT4/FP4/NF4（QLoRA 的 NormalFloat）各是什么？
- Blackwell 的 FP4：为什么是浮点不是整数？
- **和 No.138 的关系**：TurboQuant 压的是 KV cache（推理时动态生成的），这篇讲的是压模型权重（静态的）——两条正交路线

#### ② 异常值：量化的公敌，所有方法的阿喀琉斯之踵
- 什么是异常值？为什么大模型里有少数通道数值大几十倍？
- LLM.int8()（Dettmers 2022）：第一个正式提出异常值问题的
- SmoothQuant：把异常值从激活"搬"到权重上（数学等价变换）
- AWQ 的做法：不搬，直接给重要通道更高精度
- TurboQuant 的做法：旋转揉匀（No.138 已讲）
- SpinQuant（Meta）：也是旋转，但旋转矩阵是学出来的不是随机的
- **核心观点**：异常值问题从来没被"解决"，只是被不同方法以不同方式绕过去了

#### ③ KV cache 量化专题：推理显存的真正战场
- 接 No.137 Twilight（砍 token 数）+ No.138 TurboQuant（压 bit 数）
- KIVI（ICML 2024）：Key 2-bit + Value 2-bit，非对称量化
- KVQuant：per-channel 量化 + 残差编码
- Gear：低秩近似 + 稀疏矩阵处理异常值
- CacheGen：把 KV cache 当视频压（时序冗余）
- MQA / GQA / MLA：不压 bit，压 head 数——另一条省显存的路
- **和 No.364（KV Cache 排期）合并，不重复写**

#### ④ 量化的工程实践：llama.cpp / vLLM / TensorRT-LLM 里到底在用什么？
- llama.cpp：GGUF 格式，Q4_K_M / Q5_K_S 等名字到底是什么意思？
- vLLM：服务端推理框架，用 AWQ/GPTQ/FP8
- TensorRT-LLM：NVIDIA 官方，FP8/INT4-AWQ，和硬件绑定
- exllamav2：EXL2 格式，混合精度量化
- 实测对比：同一个模型，不同量化方案的 PPL / 速度 / 显存
- **选型指南**：本地跑用 GGUF，服务端用 AWQ/FP8，NVIDIA 全家桶用 TRT-LLM
...
...
58: 优化器——梯度下降的「油门」和「刹车」（SGD→Adam、显存计算、穷人方案）
