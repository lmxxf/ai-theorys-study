...
def build():
    ...
    s_bignum(prs, "① 同一个测试：它为了完成任务而采取不正当手段的比例",
             [("22%", "什么都不做\n（基线）", GREY),
              ("72%", "往「绝望」那个\n方向推一点", RED),
              ("0%", "往「平静」那个\n方向推一点", GREEN)],
             footer="推的幅度只有 0.05　—　轻轻拨一下旋钮，行为就从两成变成七成",
             note_text="测试场景：模型在一个模拟环境里执行任务，任务失败会有严重后果，同时环境里留了一条不正当的出路。测的是它走不走那条路。\n⚠️ **不主动展开那条出路具体是什么**——那个设定是把正当选项都堵死、只留一条歪路，讲细了容易被质疑，也容易被听成科幻。有人追问就说「是模拟环境里的一种不正当手段」，然后拉回「能调」这件事。\n171 个情绪方向，第一主成分对应「好受还是难受」，跟心理学的情绪环状模型对得上——没人教它，是它自己从人写的文字里长出来的。\n⚠️ 别说成 persona vectors，那是另一篇。")
    ...
    s_bignum(prs, "② 同一批题：背下来只要一千步，真会要一百万步",
             [("1 千步", "训练题\n全背下来了", BLUE),
              ("10 万步", "考试成绩\n还在瞎猜", GREY),
              ("100 万步", "突然\n就会了", GREEN)],
             footer="题目是一种特殊的除法，全套不到一万道，拿一半来训练",
             note_text="⚠️ 说「瞎猜的水平」，不说「零」——这类题随机猜约 1%。\n⚠️ 题数是按论文给的取值范围算的（模 97 的除法，被除数 97 种、除数 96 种，约 9300 道，训练用一半）。论文本身没写这个数，讲的时候说「按它给的范围算下来」。\n步数三个都是论文明写的。\n背会和真会之间隔着一道坎。内部发生的事：从「记住每道题的答案」变成「找到了那条规律」。")

    s_bullets(prs, "② 还有三个细节，比那三个数字更有意思", [
        "**模型很小**：两层，四十万参数——不是大模型，是个玩具",
        "**训练数据越少，开窍越晚**：在某一段，每少 1% 的数据，开窍要多等四五成的时间",
        "**而且不是每次都会开窍**：论文里有一道题，喂到 95% 的数据也没开窍，模型就把答案背下来了",
    ], size=21, note_text="模型：2 层 decoder-only Transformer，宽 128，4 个注意力头，约 40 万非嵌入参数。这条要说——老师会以为需要很大的模型才有这种现象。\n数据越少开窍越晚：原文在 25% 到 30% 那一带，训练数据每减少 1%，中位泛化时间增加 40% 到 50%。\n没开窍那道题：x³+xy²+y (mod 97)，任何数据比例、给到 95% 都没泛化，「模型实际上只是把训练集背下来了」。\n**第三条最值得讲：开窍不是必然的。**")
