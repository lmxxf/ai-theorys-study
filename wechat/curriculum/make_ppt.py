...
def s_bignum(prs, title, items, footer="", note_text=""):
    ...
    for num, label, col in items:
        ...
        txt(s, x + 0.1, CT + 0.42, cw - 0.2, 1.0, num, nfs, col, True, PP_ALIGN.CENTER)
        txt(s, x + 0.1, CT + 1.62, cw - 0.2, 0.95, label, lfs, GREY, False, PP_ALIGN.CENTER)
        x += cw + gap
    if footer:
        txt(s, 0.85, CT + CH + 0.35, 11.6, 1.0, footer, 22, YELLOW, True, PP_ALIGN.CENTER)
    if note_text:
        note(s, note_text)
    return s
...
def build():
    ...
    s_bignum(prs, "3.2　换个难度就掉十三个点",
             [("86%", "修正版\n人类基线约 72%", GREEN),
              ("72.6%", "更难的版本\n每题两百多步", RED)],
             footer="说明它还没真学会，是在题型里熟练",
             note_text="⚠️ 三个版本别混：OSWorld 原版 / OSWorld-Verified（现在的通用口径）/ OSWorld 2.0（另一套协议）。\n86% 是 Verified，72.6% 是 2.0。")

    s_bullets(prs, "同一年，同一批模型，另一件事", [
        "2025 年 5 月，DeepMind 的 AlphaEvolve 找到一种算法",
        "用 **48 次乘法**算两个 4×4 复数矩阵的乘积",
        "比 1969 年 Strassen 的 49 次**少一次**——那个纪录保持了 **56 年**",
    ], size=22, note_text="少一次听上去微不足道，但这是全世界数学家找了半个世纪没找到的那一次。\n⚠️ 说准：是复数矩阵这个特定设定，而且是「找到一个更好的构造」不是「证明了一个定理」。矩阵乘法的指数仍是开放问题。")

    s_twocol(prs, "这不是矛盾，是这一代 AI 的真实形状",
             "它能做到的",
             ["找到半个世纪没人找到的",
              "矩阵乘法构造",
              "",
             ...
             note_text="这就是莫拉维克悖论的当代版本。\n⚠️ 顺带说成功率：那 50 多个问题里约 75% 只是复现了已知最好解，20% 才是真正改进。八成情况它做的是追平，不是超越。\n⚠️ 陶哲轩还指出 AlphaEvolve 可靠的原因：LLM 只负责提出变异，验证器是人写的代码，不会产生幻觉。AI 负责猜，人写的程序负责判。")

    s_bullets(prs, "★ 而且「AI 解决了难题」这类标题要打折", [
        "2025 年 10 月，OpenAI 高管发帖：GPT-5 **解决了 10 个未解的数学难题**",
        "十万阅读。维护那个问题库的数学家出来澄清——",
        "标着「未解」的意思只是**「我本人不知道有论文解决了它」**",
        "GPT-5 做的是**找到了已发表的文献**：是检索，不是发现",
        "另一家 AI 公司的负责人公开回了两个字：**「这很尴尬」**",
    ], size=21, note_text="OpenAI 研究员随后删帖道歉。回复「This is embarrassing」的是 DeepMind 的 Demis Hassabis。\n给老师的一句：看到「AI 解决了 X」的标题，先问一句——**它是证明了，还是搜到了？**\n时间紧可跳过这页。")
