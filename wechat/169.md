【一个token的旅程】DeepSeek V4 预测"中国的首都是"的全流程
...
```text
- 非 RoPE 部分：448 维，用于内容相似度
- RoPE 部分：64 维，加上旋转位置编码
```

注意这里最容易算错：`head_dim=512` 不是"512 维 latent + 64 维 RoPE"，而是**每个头总共 512 维**，其中 64 维拿来做 RoPE，剩下 448 维做非 RoPE 内容匹配。
...
**为什么 Q 要低秩压缩？** 因为从 7168 维直接投影到 65536 维（128 头 × 512 维）需要一个巨大的矩阵 `[7168, 65536]`，约 4.7 亿参数。用两步低秩分解（7168→1536→65536），参数量降到 7168×1536 + 1536×65536 ≈ 1.12 亿，省了 76% 左右。

**3.3.2 KV 的投影**

V4 的 KV 是 MLA 风格——只投影出 1 份 512 维 latent，其中末尾 64 维带 RoPE：
...
**5 个 token 太短了。** HCA 层完全凑不满一个压缩块。CSA 层能凑出 1 个压缩块，但它只是一条额外摘要，真正的信息主体仍然在 128 token 滑动窗口里的原始 KV。

凑不满的怎么办？**主要走滑动窗口。**

滑动窗口大小是 128。我们只有 5 个 token，5 < 128，所以全部 5 个 token 都在滑动窗口范围内——**每个 token 都能直接看到原始 KV**。
...
**所以对短输入来说，V4 的注意力行为接近标准的因果注意力。** HCA 完全不起作用；CSA 虽然能产生 1 条压缩摘要，但在 5 个 token 全都落进滑动窗口的情况下，这条摘要几乎没有存在感。CSA/HCA/Indexer 这些机制真正是为长上下文设计的。

这很合理。5 个 token 的上下文不需要"压缩"——全看就行了，也看得起。

**3.3.5 注意力计算**

既然短输入主要走滑动窗口，核心注意力计算可以先按标准因果注意力理解：
...
还记得 hc_pre 那个 (n+1)×(n+1) 双随机矩阵 W 吗？hc_post 用的是同一个 W 的另一部分：
...
这就是 mHC 比标准残差连接灵活的地方：标准残差是 `h = h + attn_out`（1×1 的"矩阵"，永远是 [1,1]），mHC 是 5×5 双随机矩阵，4 条残差路径和层输出自由混合，但谱范数 ≤ 1 保证不爆炸。
...
`swiglu_limit = 10.0` 是一个工程保护——不是截断最终的 hidden，而是在相乘之前限制 `gate` 和 `up` 的范围，防止 FP8/FP4 量化时中间值溢出。
...
第二步：lm_head 投影到词表
  h[:, -1, :] [1, 7168] × W_lm_head [7168, 129280] → logits_last [1, 129280]
```
