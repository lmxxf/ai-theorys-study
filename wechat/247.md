...
246 期《MoE 路由——前 3 层查表，后 40 层打分》（ https://mp.weixin.qq.com/s/EwUO9tVfrpCrcwsyPI42uw ）讲了路由怎么选出 top-6 专家。选完之后呢？6 个路由专家加 1 个共享专家，7 次 FFN forward。每个专家里面是什么？SwiGLU。
...
顺带笑一个：既然 2048 < 4096，**W_up 这个"up"在 V4 里其实是在往下压**。这名字是 dense 时代的化石——LLaMA 里它真的在升维（4096→14336），名字起对了；到 MoE 专家这儿方向反了，但全世界的代码照旧叫 gate_proj / up_proj / down_proj。**命名跟的是"流程里的角色"，不是实际方向**——读代码时别被名字骗，看形状注释才作数。
...
![SiLU vs ReLU vs GELU](assets/247/silu-curve.png)

看图记两个特征就够了：**负半轴那个小尾巴**（最低点约 -0.28，这就是"梯度不死"的意思——ReLU 在这里是条死平线），和**正半轴渐近贴住 y=x**（大信号几乎原样通过，门全开时不失真）。GELU 和 SiLU 几乎重叠——所以选谁看的不是形状，是计算成本。

⚠️ 顺便钉死一个常错的细节

**两路投影里，gate 那一路过 SiLU，up 那一路不过。** 我每次记反的概率大约一半——直到想清楚命名逻辑：

- "gate"——「门」，门当然要做激活——开还是关，就是它的全部职责
- "up"——「内容」那一路（名字别当真，上一节说了它其实在降维），内容是原料，原料不需要被激活函数捏一下——捏它的是门

一句话：**门管开关所以过激活，料管内容所以直通**。这么记就不会反。
...
- **239 期 act_quant**（ https://mp.weixin.qq.com/s/OozibTbTLlDiBhh57NOv0A ）—— 把激活值 x（bf16）按块量化到低精度：每块取 absmax 当 scale，clamp 到表示范围再 cast。分工记清楚：**激活量化到 fp8（e4m3），权重存的才是 fp4（e2m1）**——fp4 只有 4 bit（2 位指数 1 位尾数 1 位符号），最大约 ±6，拿来存训练好的静态权重够用，拿来装动态范围大的激活就太挤了，所以激活多留 4 bit。
- **240 期 fp_gemm**（ https://mp.weixin.qq.com/s/7ROESDoxhexsMWL9HVz8wA ）—— Triton 的 `tl.dot_scaled` 在硬件层把"fp8 激活 × fp4 权重"算成 fp32 累加，比手动把 fp4 解包成 fp16 再算快好几倍。

把这两块拼到 SwiGLU 上（clamp 的位置按官方 model.py 核过，量化步骤是实验平台的流水线）：
...
attention 那边的投影矩阵反而没这么激进，原因两头（官方没明说，这是我们按计算特性推的）：

- **收益侧**：那些矩阵本来就被低秩压缩瘦过身，参数占比小，省一倍没多少肉；
- **风险侧（更要紧）**：FFN 是"求和"型计算——几千个乘积加起来，量化噪声互相抵消，加上 256 个专家的高冗余，粗颗粒天然安全。attention 是"选择"型计算——Q·K 的分数要过 softmax，**指数会把小差距放大成大差距**：两个 key 本来差 0.3，噪声挪 0.2，注意力分配可能整个换人。错了不是"输出糊一点"，是"看错了地方"。而且低秩压缩后的表征信息密度更高、冗余更低，量化伤得反而更重。

一句话：**量化的火力对准"参数肥、计算糙"的地方——FFN 两条全占，attention 两条全不占。**
...
模型发布页上总有一句"总参数 XXB / 激活参数 XXB"——33 期讲 V3 时那句"671B 总参数 / 37B 激活"（ https://mp.weixin.qq.com/s/SGAt3w3d1C3icAB3JbgDYw ）就是这个格式。**"激活参数"这个数是怎么来的？拿今天的专家自己算一遍就明白了。**

V4 Flash 每个 token 在一层里要过的 FFN：
...
7 个专家 ≈ **176M 参数**。这就是一层 FFN 的"激活参数"——**这个 token 实际摸到的权重**。而这一层躺着的 FFN 总参数是 257 个专家 × 25M ≈ **6.4B**——每个 token 只摸了其中 2.7%，剩下 250 个专家这一步完全不参与计算、不花一个 FLOP。

对一下官方口径：V4 Flash 模型卡写的是 **284B 总参数、13B 激活**。咱们手上这笔账正好能对上大头——FFN 总量 43 层 × 6.4B ≈ 276B（总参数几乎全躺在专家里）；FFN 激活 43 层 × 176M ≈ 7.6B，再加上 attention、embedding 这些每个 token 都要过的部分，凑到 13B。**发布页那两个数不神秘，就是这么一层层加出来的。**

176M 是什么概念？**恰好等于一个 8B 级 dense 模型的单层 FFN**（LLaMA-3-8B：hidden=4096、intermediate=14336，三个矩阵也是约 176M）。注意这不是比武，是对账：**284B 的 V4 Flash，每个 token 在 FFN 上花的算力，跟一台 8B 小模型一个价**。

所以 MoE 的核心账就一句话：**记忆按总参数算（256 个专家分头存 256 种语义模式，谁都不用挤），算力按激活参数算（每个 token 只为它摸到的 7 个专家付钱）**。dense 模型记多少就得每步全算多少，记忆和算力锁死在一起；MoE 把这两个量解耦了——这是它存在的全部理由。
...
表里还没带过链接的三站传送门：242 期 Indexer（ https://mp.weixin.qq.com/s/iRTyUmab0SXS3HnzxkcAGw ）、243 期 sparse_attn（ https://mp.weixin.qq.com/s/X1FIW7H1WEmI4mAi6mYm_g ）、244 期 mHC Sinkhorn（ https://mp.weixin.qq.com/s/IpmdCFVbfZwVwmQno815LA ）；239、240 的链接在第五节，246 的在开篇。
...
这个仓库把系列讲的算子全部用纯 PyTorch + Triton 重写了（kernel_sm121.py），不依赖工业级推理框架（TileLang、DeepGEMM、vLLM 全用不上），就是为了让消费级硬件能跑 284B 的 V4 Flash。每个 kernel 不到 200 行，照着这七期文章对应着读，比看论文直观。
