...
## 十五、⭐ 候选题（08-21 Zero 立）：一次完整的后训练，到底存了什么信息、存多大

**Zero 原话**：「学习一下一次完整的后训练，到底存了什么信息，存多大」。账本体（277 KV 账 / 241 token 账同款笔法）。

**账目草单（写时逐项核数）**：
- **权重本尊的份数**：policy + reference + critic（如有）+ 老师们（V4 十几个、每个可达万亿参数）——各多少字节、FP8/FP4 存法
- **优化器状态**：Adam 一阶+二阶矩 = 权重的 2 倍 FP32（Muon 呢？K3 用 Muon，状态更小——可对比）；K3 训练态下沉 NVMe 那笔就是它
- **checkpoint**：多久存一次、存几份、一份多大；slime Delta Weight Sync 的差量+压缩是这笔账的省法
- **rollout 轨迹**：一轮几千条 × 每条几十万 token，logprob 要不要跟着存（Missing Old Logits 那篇：不存会出算法问题）
- **KV cache**：K3 外置 CPU 池那笔；1M 上下文一条轨迹的 KV 多大（277 期算过单条，这里乘并发）
- **V4 老师侧**：末层 hidden states 缓存 vs 全 logits 物化（"即使写到磁盘也存不下"——129280 词表 × 百万 token × FP32 可以现场算给读者看）
- **沙箱侧**：150 万个镜像、5100 万沙箱的快照/日志；WAL 每 token 一条
- **对照锚点**：预训练存什么（就一份权重+优化器+数据）vs 后训练存这一大家子——后训练是"存储结构最复杂的训练形态"
- 可接：287（车间）、277（KV 账）、284（老师调度）
