...
这里先补一点 V4 的背景知识，不然后面 `d` 的取值会显得突兀。V4 用的是 MLA（多头潜在注意力）——Q 不是从模型的隐藏维度直接投影出来的，中间多插了一层压缩：

```text
hidden_size (4096)
    │  wq_a（降维）
    ▼
q_lora_rank (1024)   ← 压缩过的 Q，一份共享的低秩表示
    │                   KV cache 存的就是这个压缩后的 1024 维，省显存省带宽
    ├─ wq_b（主路径用）    ──▶ 64 head × 512 head_dim = 32768 ── 主 attention 的真 Q
    │                          （真正用来算 attention 输出，维度大是为了保留精度）
    │
    └─ wq_b（Indexer 自己一套）──▶ 64 head × 128 head_dim = 8192 ── Indexer 打分专用的小 Q
                               （只用来打分排序，不需要还原精确数值，维度可以小很多）
```

主路径 attention 和 Indexer 各自有一份独立的 `wq_b`，从同一份 1024 维压缩表示出发，展开到不同的宽度。所以接下来表里的 `index_n_heads`=64、`index_head_dim`=128，说的是 Indexer 自己这份独立展开出来的 Q，跟模型整体的 4096 维隐藏层没有直接的大小关系——8192 比 4096 大不奇怪，它是从压缩过的 1024 维展开出来的，展开到多宽是设计者自己定的。

接着第一节那次实际跑的 prompt（S=17832）为例，把范围填进去：

| 下标 | 含义 | 这次实际取值 |
|---|---|---|
| `b` | batch，一次处理几条 prompt | 1 |
| `s` | query 在序列里的位置，取值范围 0~S-1 | S=17832，所以 s 有 17832 种取值 |
| `h` | Indexer 的 head 编号 | `index_n_heads`=64（TP，Tensor Parallel 张量并行=2，两台机器各分一半，单卡只算其中 32 个） |
| `t` | 压缩 KV 在 T 条里的位置，取值范围 0~T-1 | T=S/4≈4458，所以 t 有 4458 种取值 |
| `d` | 每个 head 的向量维度，公式里被 Σ 加总掉，不出现在输出里 | `index_head_dim`=128 |

所以 `score[b, s, h, t]` 完整展开的形状是 `[1, 17832, 64, 4458]`——后面第五节会算到，这四个维度的取值范围直接相乘，就是那个 20.3 GB 中间张量的由来。
...
顺带说一句和这套设计哲学正好对称的一个坑——**Indexer 选得准不准，取决于你要找的东西是不是和当前内容相关**。它靠 Q·Kᵀ 打分，本质是找语义邻居，那些和当前 query 内容正交的信息（比如一条格式约束、一条系统指令），Indexer 结构上就没有信号能指向它——不是精度不够漏选，是**打分机制从设计上就不覆盖"内容无关但重要"这一类东西**。这也是为什么长文本测评（MRCR，多轮指代消解，专测"在一堆无关内容里能不能精确捞出某个键值事实"这类）分数能很高——因为测的都是「内容锚定」的键值召回，Indexer 的强项——但一条脱离上下文语义、只需要"始终生效"的约束，反而更容易在 CSA 层被路由绕开、在 HCA 层被池化稀释掉。挑 top-k 这件事，挑的是"像"，不是"该记住"。

这不是纯推理——之前拿 V4 做过一次翻译任务，开头要求 **"一段英文一段中文"** 交替输出，长文本写到后半段这条格式指令就丢了，翻回去查才发现：指令本身还在，只是压缩层和 Indexer 都没有理由把它捞出来参与当前这几个 token 的计算。往后凡是"这条规则要从头管到尾，但内容上和当前在算的东西没关系"这类约束，在 CSA/HCA 这套架构上都要留个心眼——最稳的办法是在长输出中间隔一段就重复提醒一次，别指望它自己被"看见"。
...
def Indexer_forward(x, qr, start_pos, offset):
    ...
    # head_dim 里最后 rd=64 维专转 RoPE（rope_head_dim 全局固定 64），
    # 剩下的是纯内容——不转的话点积只反映"内容像不像"，感知不到 query
    # 和压缩 KV 差多远，选出来的 top-512 就少了"离得近该更优先"这层。
...
S=17832 是我们实际跑的 prompt 长度，n_heads=64 是 V4 Indexer 的总 head 数（TP=2 时单 rank 32 个），ratio=4 所以 T=S/4≈4458。**单层 Indexer 中间张量 20.3 GB。** 43 层里有 21 层是 CSA——还好这是中间张量，每层算完就释放，不是累积的。但单层就 20 GB，统一内存 128 GB 的机器跑两三次就爆——227 期《DeepSeek V4 推理：一次 OOM 的经验——注意力一波算不完》（ https://mp.weixin.qq.com/s/XM-xSvJFXh1Y49YfvJ49DQ ）记过同一类"单层看着不大、累积起来炸内存"的坑。

S 再翻 7 倍到 131072（这个实验平台自己试过的一个 `max_seq_len` 配置，不是 V4 论文标称的上限——V4 Flash 官方支持的上下文实际超过 100 万 token，131072 只是我们本地测试用的一档），单层就是 274 GB。这数字直接说明一件事：**这个 einsum 在大上下文下，根本不能实例化整张表。**
