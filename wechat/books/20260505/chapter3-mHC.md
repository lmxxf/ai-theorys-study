...
V4 同时引入了 Muon 优化器（矩阵级正交化更新）和 mHC。两者存在协同效应：Muon 的正交化让更新方向均匀分散，可能引入注意力数值不稳定问题（Kimi K2 训练中观察到注意力分数飙升到 1000 以上）。mHC 的双随机矩阵约束从架构端控制了信号增益，使得 V4 无需在优化器端额外添加 QK-Clip 等数值裁剪措施。V4 的注意力架构还允许直接对 Q 和 KV entries 做 RMSNorm，进一步抑制注意力分数爆炸。
...
mHC 已在 DeepSeek-V4（1.6T 参数，33T tokens 预训练）中得到大规模部署验证，与 MoE、MLA、CSA/HCA 混合注意力、Muon 优化器共同构成 V4 的核心架构组件栈。
