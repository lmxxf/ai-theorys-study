...
1. **mHC**（流形约束超连接）：将层间信号放大倍数从 HC 的 $10^3 \sim 10^5$ 压缩至约 1.6
...
- **mHC**：将 HC 的信号放大从 $\sim 3000\times$ 压缩至 $\sim 1.6\times$（双随机矩阵约束保证谱范数 $\leq 1$）
...
3. **系统层面**：在 DeepSeek V4 中，99.9% 参数使用 Muon，配合 mHC 和 RMSNorm 从架构端解决数值问题，不需要优化器端的额外裁剪
