...
两个规格共享相同的架构创新，区别仅在于参数规模和层数。V4-Pro 面向旗舰推理场景，V4-Flash 面向高性价比部署场景。两者均支持 1M token 上下文窗口。
...
- **CSA（Compressed Sparse Attention）**：先将 KV cache 沿序列维度压缩——每 $m=4$ 条 KV 通过学习的加权池化压成 1 条"智能摘要"，再使用 Lightning Indexer 从压缩后的 KV 中选出 top-$k=1024$ 条进行注意力计算。V4-Pro 配置为 64 个 Indexer 头（head dim = 128），整条 Indexer 路径全程 FP4 精度。CSA 负责"从全局历史中精选最相关的细节"。
...
两种注意力层交替排列，使模型同时具备全局检索能力和局部建模能力，在 1M 上下文长度下将注意力计算的实际复杂度从 $O(n^2)$ 降至近似 $O(n \log n)$。
...
| 参数类别 | 优化器 | 超参数 |
|---------|--------|--------|
| Embedding / Prediction Head / RMSNorm / mHC 静态偏置 | AdamW | $\beta_1=0.9$, $\beta_2=0.95$, $\varepsilon=10^{-20}$, weight decay $=0.1$ |
| 其余所有参数（包括 MoE 专家、Attention 投影等） | Muon | momentum $=0.95$, weight decay $=0.1$, RMS rescale $=0.18$ |
...
该策略的动机是：稀疏注意力的路由模块（Lightning Indexer）需要基于有意义的表征进行块选择；若从随机初始化开始就启用稀疏注意力，路由器和表征会陷入"鸡生蛋"困境——差的表征导致差的路由，差的路由阻碍表征学习。先用全注意力训出基础表征，再引入稀疏机制，避免了这一冷启动问题。
