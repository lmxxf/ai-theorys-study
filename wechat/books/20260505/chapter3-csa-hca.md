...
**MLA**（DeepSeek-V2, 2024）开辟了全新维度——不砍头数，砍每条 KV 的维度。其核心思想是将 Key 和 Value 联合压缩为一个低维 latent 向量：

$$
\mathbf{c}_t = \mathbf{h}_t \mathbf{W}_{\text{down}}, \quad \mathbf{W}_{\text{down}} \in \mathbb{R}^{d \times d_c}
$$

其中 $d = 7168$（隐藏维度），$d_c = 512$（latent 维度）。推理时只存 $\mathbf{c}_t$，通过将还原矩阵吸收进 Query 投影和输出投影，**latent 从头到尾不需要被解压为显式的 K 和 V**。V4-Pro 的 `num_key_value_heads = 1, head_dim = 512` 即为此配置。
...
其中上标 $a$ 表示"当前块"角色，上标 $b$ 表示"重叠到下一块"的角色（详见 3.x.2.2 节）。$\mathbf{W}^{aKV}, \mathbf{W}^{bKV} \in \mathbb{R}^{d \times d_c}$ 为 KV 投影矩阵，$\mathbf{W}^{aZ}, \mathbf{W}^{bZ} \in \mathbb{R}^{d \times 1}$ 为标量压缩权重投影。
...
压缩后的 CSA KV cache 仍有 $N/m$ 条（50K 上下文约 12500 条）。模型并非全部使用，而是通过 Lightning Indexer 选出最相关的 top-$k$（$k = 1024$）条进行注意力计算。

Lightning Indexer 是一条**完全独立**的打分路径，与主路径不共享参数：

$$
\mathbf{q}^I = \text{FP4}\!\left(\text{Hadamard}\!\left(\mathbf{h}_t \mathbf{W}_Q^I\right)\right)
$$

$$
\mathbf{k}^I_i = \text{FP4}\!\left(\text{Hadamard}\!\left(\mathbf{C}_i^{\text{Comp}} \cdot \mathbf{W}_K^I\right)\right)
$$

$$
\text{score}_i = {\mathbf{q}^I}^\top \mathbf{k}^I_i, \quad \text{idx} = \text{top-}k(\text{score}_{1:N/m})
$$

其中 $\text{Hadamard}(\cdot)$ 为 Hadamard 正交旋转——将向量分量的能量均匀分散，使后续 FP4 量化（仅 16 个可表示数值）的精度损失最小化。

**Indexer 的参数配置**：$n_h^I = 64$ 个 Indexer 头，压缩维度 $c^I = 128$。整条 Indexer 路径全程 FP4 运算——排序只需保持相对顺序，对绝对精度容忍度高。

**主路径与 Indexer 的精度分离**：主路径的压缩 KV cache 存储为 FP8（压缩已有损，精度不能再砍），Indexer 全程 FP4。两条路独立运行、互不污染，可真并行。
...
2. **选择**：Lightning Indexer 从 $N/m$ 条压缩 KV 中选出 top-1024 条
3. **注意力**：128 个 Query 头对选中的 1024 + 128（滑窗）条做标准注意力计算
...
| 稀疏选择 | top-1024（Lightning Indexer） | 无（压缩后全看） |
...
- **CSA 层**：精选细节（1024 条） + 最近精细上下文（128 条）
...
其中 $c = 512$ 为每头维度，$d_g = 1024$ 为每组输出维度。16 组输出拼接后恢复为 $16 \times 1024 = 16384$ 维（或经额外投影回到 $d = 7168$）。分组投影减少参数量的同时保留组内头间的交互能力。
...
**CSA 层**（$m = 4$，top-1024 选择后仅需存全量压缩 KV 供 Indexer 使用）：
...
| CSA/HCA（V4） | 228 MB | 0.003 |

**总压缩率：99.7%。** 从 81.5 GiB 压缩至 228 MB。
...
1. **序列维度的双精度压缩**：CSA（$m = 4$，重叠，top-1024 精选）提供细粒度选择性访问，HCA（$m' = 128$，不重叠，全看）提供低成本全局概览，两者层间交替互补
2. **压缩与选择的解耦**：Lightning Indexer 独立于主路径运行，FP4 全程量化，消除精度-效率的耦合约束
3. **训推一致性**：从训练第一步即启用压缩注意力，模型的内部表征与压缩后的世界"共同生长"，不存在 DSA 式的训推分裂
4. **极致存储效率**：结合 MLA 的维度压缩和 Compressor 的条数压缩，50K 上下文的 KV cache 从 81.5 GiB 压缩至 228 MB（99.7%），使 1M 上下文的工程部署成为可能

从更根本的角度看，CSA/HCA 能实现 99.7% 的压缩率而基本不损失模型能力，根源在于**自然语言的有效信息本身是低维的**——7168 维隐藏空间中，语义路径集中在几百维的低维流形上。V4 的全部注意力优化归结为一件事：只沿着流形存路径，不存流形以外的空气。
