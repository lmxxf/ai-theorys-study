...
对于 DeepSeek-V3.2（MLA 架构），该比值约为 **22 GB/PFLOP**。而对于 KV Cache 体积更大的稠密模型（如 Qwen2.5-32B FP16），该比值可达 267 GB/PFLOP，远超硬件的 I/O-计算平衡点——这也从侧面体现了 MLA 压缩的巨大优势。
...
DualPath 基于 DeepSeek 内部推理框架实现，集成 FlashMLA、DeepGEMM 和 DeepEP 组件，存储后端采用 3FS（无内部 DRAM 缓存），I/O 层使用 io_uring 式内核绕过技术。在现有推理框架基础上约 5,000 行修改。
...
离线场景：从 2P4D 扩展至 48P96D（24 倍节点），处理 48K Agent 的总耗时仅增加 1.1%（3,167s → 3,201s），实现了近完美的线性扩展。在线场景：从 2P4D 扩展至 44P88D（22 倍节点），吞吐量从 0.4 APS 提升至 8.8 APS（**22 倍**）。TTFT（Time To First Token）在扩展过程中基本保持不变，调度器 CPU 开销低于 10 核。
...
当任务完成时间（JCT）因外部等待（如工具调用延迟）增大 $r$ 倍时，工作集按 $r^2$ 扩展，这一二次方关系对存储系统的容量规划具有重要指导意义。
...
| TailorKV | 逐层混合压缩（量化 + 稀疏检索） | 压缩 KV Cache 体积，与 DualPath 正交可组合 |
