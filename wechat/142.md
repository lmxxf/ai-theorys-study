【Claude Code 源码泄露】卧底模式、脏话检测、反蒸馏、和一只电子宠物
...
更关键的是：**这个壳子不开源。** Anthropic 用了一堆开源组件（Bun、React、Ink、Zod），但壳子本身闭源商业化。**模型是核心竞争力，壳子不是。但 Anthropic 把壳子也锁了，不是因为壳子值钱，是因为壳子是收费的入口。** 你不用官方壳子，他就没法按月收你钱、没法追踪你的用量、没法在 API 响应里注入反蒸馏假数据。正版二进制才能调 API，第三方客户端可能被拒。
