...
OpenClaw 是 2026 年增长最快的开源 AI Agent 框架（GitHub 163K stars），但它有 40 万行 Node.js 代码和一堆 Docker 依赖——移植到鸿蒙？想都别想。

好在有人替我们做了第一步简化：NanoClaw，OpenClaw 的极简重写版，约 3500 行 TypeScript，5 个文件，把 Agent 的核心功能提炼出来了：ReAct 循环、工具调用、持久记忆、定时任务。但它仍然跑在 Node.js + Docker 上，用 Claude Agent SDK——鸿蒙上这些还是没有。
...
**代码是 Claude 写的，坑是一起踩的。** 我负责说"要什么"和"不要什么"，Claude 负责把 NanoClaw 的 TypeScript 翻译成 ArkTS，处理鸿蒙 API 的各种差异。
...
ArkTS 本来就是 TypeScript 的超集，这些语法特性 TypeScript 都支持，硬砍掉是平台方自己的选择。砍掉之后开发者只能手写替代方案，开发效率打折扣，生态也建不起来。遥遥领先。
