...
**DeepSeek V3 完全绕过了这个噩梦。** 他们不调α，而是开创了**无辅助损失**策略：
...
**结果：** 训练和推理都不丢弃token。整个训练过程**零loss spike，零回滚**。
...
这就是为什么DeepSeek V3训练成本仅557万美元（对比GPT-4估计的5000万-1亿美元）。他们不是优化问题——而是**消灭了问题**。
...
DeepSeek V3 明确**强调数学和编程样本**——这就是它推理能力强的原因。
...
标准 Transformer 训练**每个位置预测一个 token**。对于 14.8T token 的数据集（DeepSeek V3 的训练语料），这意味着 14.8 万亿次前向传播。
...
DeepSeek V3 将预测扩展到每个位置的**多个未来 token**：
...
**关键创新：** 与并行 MTP 方法不同，DeepSeek 的 MTP 通过顺序预测额外 token 来**保持因果链**。
...
| **Auxiliary-Loss-Free** | DeepSeek's method to bypass α tuning | Bias term instead of loss |
| **DualPipe** | DeepSeek's pipeline optimization | Overlaps compute & communication |
| **MLA** | Multi-Head Latent Attention | Compresses KV cache |
| **FP8 Training** | Lower precision than BF16 | DeepSeek's cost reduction secret |
| **DeepGEMM** | DeepSeek's open-source FP8 GEMM | Tile-wise quantization |
| **MTP** | Multi-Token Prediction | Predicts multiple tokens at once |
| **Data Mixing** | Every batch contains all data types | Prevents sequential forgetting |
| **Curriculum Learning** | Easy → hard training order | Better generalization |
| **Data Repetition** | High-quality data repeated N epochs | Limited by Chinchilla |
| **Deduplication** | Remove duplicate training samples | Data quality step |
| **Quality Scoring** | Rate training samples by quality | Filter low-quality data |
...
所有那些掌握路由崩溃恢复、NaN调试和4D并行协调的工程师...本质上是在给一扇**没有墙的门**装一把**非常昂贵的锁**。
