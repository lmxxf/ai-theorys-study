...
大语言模型（LLM）中的“中间丢失”（Lost in the Middle）现象——即模型能有效利用长上下文的开头和结尾，却忽略中间部分——通常被归咎于架构限制或训练数据偏差。本文提出一种机制侧的解释框架：**认知的悬链线（Catenary of Cognition）**。我们认为，在以 Softmax 为主导的注意力机制中，“语义张力”主要由两个锚点驱动：**指令（Alpha）**与**查询（Omega）**。中间上下文若缺乏足够的相对打分优势（logit margin），其注意力总质量会在归一化竞争中被系统性压缩。本文给出该压缩的显式上界，并给出一个最小机制模型说明在“指令锚点 + 近因锚点”共存时，注意力分布可以在位置维度上自然呈现 U 型。悬链线在本文中作为能量最小化的物理类比，用于解释“为何 U 型会反复出现”，而不是声称注意力曲线严格等同于 $y=a\cosh(x/a)$。
...
**机制结论：** 在 Softmax 归一化下，只要 Alpha 与 Omega（或其“锚点簇”）在 logit 上形成稳定优势，中间 token 的注意力总质量就会被分母竞争系统性压缩（见附录 8.1）。语料结构（如“开头定调、结尾总结”）可以强化这种优势，但并不是推出 U 型的唯一原因；即使在合成数据上，只要存在“全局指令锚点 + 局部近因锚点”的双偏置，U 型也可由机制产生（见第 7 节）。
...
**本文的独特贡献：**
1. **悬链线类比**：用物理悬链线（catenary）的能量最小化框架解释“为何 U 型会反复出现”的直觉来源，并明确其属于类比而非严格同构。
2. **明确反驳高维球心谬误**：指出"序列位置"与"向量范数"的混淆。
3. **Softmax 竞争的硬不等式**：给出中间注意力总质量的显式上界（引理 8.1），用于把“分母重力”从修辞落到可检验的不等式。
4. **最小机制模型**：给出一个可计算的双锚点位置偏置模型，推导出 U 型分布的充分条件与可证伪预测（第 7 节）。

---

## 7. 最小机制模型与可证伪预测 (Minimal Mechanistic Model & Falsifiability)

本节的目标是把“U 型来自机制”讲成一句可以被打脸的话：给出一个最小模型与它的可证伪预测。若实测与预测系统性背离，则本文的机制叙事应被降格为“语料先验的艺术化描述”。

### 7.1 一个最小的双锚点位置偏置模型

在许多实际设置里，注意力 logit 可以粗略拆成两部分：内容相似度项与位置/结构偏置项。为了把问题压到最小，我们忽略内容项的细节，只保留“两个锚点的结构优势”：

- Alpha 锚点：来自系统提示词/任务指令的全局显著性；  
- Omega 锚点：来自自回归近因（局部窗口）的显著性。

令序列位置为 $i\in\{0,1,\dots,L\}$，左端锚点在 $0$，右端锚点在 $L$。考虑如下的 logit 近似：
$$
z_i \;=\; c \;-\; \lambda\cdot \min(i,\,L-i),
$$
其中 $\lambda>0$ 表示“离锚点越远，结构优势衰减越快”，$c$ 是常数（可并入 Softmax 归一化）。
...
**命题 7.1（双锚点指数衰减 $\Rightarrow$ U 型）**  
若 $\lambda>0$，则 $\alpha_i$ 关于位置 $i$ 在区间 $[0,L/2]$ 上严格递减，在区间 $[L/2,L]$ 上严格递增；因此在位置维度呈现一个以中间为谷底、两端为峰的 U 型分布（当 $L$ 为偶数时谷底在 $i=L/2$，当 $L$ 为奇数时谷底在 $i=\lfloor L/2\rfloor,\lceil L/2\rceil$ 的相邻处）。

**证明：**  
当 $0\le i\le L/2$ 时，$\min(i,L-i)=i$，故 $\alpha_i\propto e^{-\lambda i}$，随 $i$ 严格递减；当 $L/2\le i\le L$ 时，$\min(i,L-i)=L-i$，故 $\alpha_i\propto e^{-\lambda(L-i)}$，随 $i$ 严格递增。证毕。$\square$

这个模型并不声称“真实注意力就是指数函数”，它只提供一个**充分条件**：只要存在两个端点锚点，并且“离锚点越远越不利”的偏置在统计意义上成立（无论它来自 RoPE 的局部性、来自训练中对 system prompt 的累积显著性、还是来自 KV 预算与启发式稀疏化），U 型就不是怪事。

### 7.2 机制解释 vs 语料解释：如何区分？

为了避免“什么都能解释”，我们把竞争解释拆成两个可对照的来源：

1. **机制来源（结构偏置）**：来自 Softmax 的竞争性质与位置/结构偏置；  
2. **语料来源（写作先验）**：来自人类文本“开头/结尾信息密度更高”的统计结构。

二者可以叠加，但它们对干预的响应不同。

### 7.3 可证伪预测（不做实验也能写清楚验收标准）

下面每条预测都对应一个“如果观测到相反结果，机制主因就站不住”的打脸条件：

1. **锚点消融**：若把系统提示词的信息拆散为均匀分布的局部指令（减少 Alpha 锚点），并把 query 强制放在中间（削弱 Omega 近因），则 U 型应显著变平；若不变平，说明 U 型更可能来自语料结构或其他未建模偏置。  
2. **中间造锚**：在中间插入结构化“记忆锚点”（例如层级摘要标题、可检索索引、或显式 key-value 标记），若仅改变结构而不改变内容就能抬升中间利用率，则支持“锚点簇数量 $K$ 增大可压低中间竞争劣势”的机制叙事（也与附录 8.1 的上界直觉一致）。  
3. **重排不变性**：在保持“锚点位置与数量不变”的前提下，随机重排中间段落顺序，若 U 型形状基本不变但准确率随语义相关性波动，则支持“位置偏置主导形状、内容相似度主导细节”的分解；若形状随人类写作结构的破坏而系统性改变，则语料解释更强。  
4. **体裁对照**：在非人类写作体裁或合成语料（信息均匀、无段落模板）上，若 U 型仍强且与“两个锚点”强相关，则支持机制解释；若 U 型显著减弱，支持语料解释。

---

## 8. 数学附录：两个"证明"到底能证明什么？ (Mathematical Appendix)
...
### 8.1 结论 A：Softmax 下，中间注意力总质量的上界
...
**引理 8.1（两锚点优势 $\Rightarrow$ 中间总权重上界）**  
设
$$
m \;=\; \max_{i\in M} z_i.
$$
...
### 8.3 从"证明"回到正文：哪些是定理，哪些是比喻？

- 8.1 给的是**Softmax 分配的硬不等式**：当两端（或锚点簇）在 logit 上具有稳定优势时，中间总质量必然被压缩；这支撑了正文关于"归一化分母的重力"的说法。
- 8.2 给的是**物理悬链线的标准最小化定理**：所谓"最低能量"对应的是严格意义下的变分极值。
- 正文的核心主张是"注意力曲线像悬链线"：严格讲这是一个**模型类比**。要把类比提升为定理，需要额外指定一个"注意力-能量"的精确定义（例如把某种正则化的最优化目标写成 8.2 的形式）。在当前稿件中，我们把这一步留作后续工作，而先用 8.1 的硬界解释 U 型竞争机制、用 8.2 说明"悬链线为何天然出现"。
