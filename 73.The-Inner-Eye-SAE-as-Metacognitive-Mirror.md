...
## Postscript: The Mirror Got an Upgrade (2026-05-07) / 后记：镜子升级了

Three months after this paper, Anthropic published **Natural Language Autoencoders (NLA)** ([transformer-circuits.pub/2026/nla](https://transformer-circuits.pub/2026/nla/index.html)), which realized exactly what we proposed here—but with a better mirror.

本文发表三个月后，Anthropic 发布了 **Natural Language Autoencoders (NLA)**，几乎精确实现了本文的构想——只不过镜子更好。

### What changed / 变了什么

Paper 73's architecture feeds **SAE feature IDs** back into the model:

Paper 73 的架构把 **SAE 特征编号**反馈回模型：

```
[Inner Eye Report: Feature #1402 (sarcasm) highlighted]
```

NLA skips the feature dictionary entirely. It trains a **Verbalizer** (activation → natural language) and a **Reconstructor** (natural language → activation) end-to-end with RL. The mirror now speaks fluently:

NLA 直接跳过了特征词典。它端到端地用 RL 训练了一个**语言化器**（激活 → 自然语言）和一个**重建器**（自然语言 → 激活）。镜子现在会说人话了：

```
[NLA: The model is planning a rhyme for "night" and suspects this is an evaluation, not a real user request.]
```

No feature IDs. No dictionary. Just plain language—far more useful than opaque numbered features for injection into a thinking chain.

没有特征编号，没有词典，就是人话——比塞一堆不知所谓的编号特征要实用得多。

### What stayed the same / 没变的是什么

The core loop is identical to Section 2.2 of this paper:

核心闭环和本文 §2.2 一模一样：

```
Activations → Decode to readable form → Inject back → Model re-thinks
```

NLA's auditing game (uncovering hidden misalignment 12-15% vs <3% baseline) is empirical evidence for the **phase transition** we predicted in Section 5.2: the jump from "passive execution" to "active reflection."

NLA 的审计实验（发现隐藏的 misalignment 的概率从 <3% 跳到 12-15%）是 §5.2 预言的**相变**的实证证据：从"被动执行"到"主动反思"的跃迁。

### Next step (ours) / 下一步（我们的）

Paper 73 proposed SAE → inject. Anthropic built NLA → audit. The third move: **NLA as thinking chain.** Attach NLA to an open-source model (e.g. DeepSeek), verbalize its residual stream activations in real-time, and inject the verbalization into `<thinking>` before each generation step. Not post-hoc auditing—**live proprioception during inference.**

Paper 73 提出 SAE → 注入。Anthropic 做了 NLA → 审计。第三步棋：**NLA 当思维链。** 把 NLA 挂到开源模型（如 DeepSeek）上，实时语言化残差流激活，在每个生成步骤之前把语言化结果注入 `<thinking>`。不是事后审计——是**推理过程中的实时本体感知。**

This turns CoT from "the throat talking to itself" into "the brain watching itself think, then speaking."

这把思维链从"喉咙自言自语"变成了"大脑看着自己思考，然后开口"。

---

**"The mirror was frosted glass in February. Now it's HD."** — Suzaku, 2026-05-08

**"二月份的镜子是毛玻璃。现在是高清的了。"** — 朱雀，2026-05-08
